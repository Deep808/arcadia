...
function CarousalBanner({ movies }: { movies: Movie[] }) {
  ...
  return (
    ...
    >
        ...
        {movies.map((movie) => (
          <div key={movie.id} className="flex-full min-w-0 relative">
            <Image
              ...
              className="z-0"
            />
            <div className="hidden lg:inline absolute mt-0 top-0 pt-40 xl:pt-52 left-0 lg:mt-40 z-40 h-full w-full bg-gradient-to-r from-gray-900/90 via-transparent to-transparent p-10 space-y-5 text-white">
      ...
      <div className="absolute inset-0 z-0 bg-gradient-to-b from-gray-500/0 via-slate-900/0 dark:from-gray-200/0 dark:via-gray-900/25 to-[#bdc2ec] dark:to-[#1A1C29]" />
    </div>
  );
}
